...
function currentHour() {
  // Taiwan time hour prefix e.g. "2025-04-01T14" (UTC+8)
  const tw = new Date(Date.now() + 8 * 60 * 60 * 1000);
  return tw.toISOString().slice(0, 13);
}
...
app.get('/api/slot/status/:cardId', (req, res) => {
  ...
  res.json({ totalPts: total, availablePlays, usedPlays, remainingPrizes, todayWins: wins, hourlyWins, hasWonToday });
});
...
app.post('/api/slot/play', (req, res) => {
  ...
  // Check hourly prize cap — win_time is ISO string (UTC), slice prefix for hour matching
  const hourPrefix = hour + ':'; // e.g. "2025-04-01T06:"
  const { wins: hourlyWins } = db.prepare(
    "SELECT COUNT(*) as wins FROM slot_wins WHERE win_time LIKE ?"
  ).get(hourPrefix + '%');
  ...
  // - Hourly cap reached → 0%
  // - Under cap → base 15%, but scales down as hourly wins increase
  //   0 wins this hour → 15%, 1 win → 10%, 2 wins → 5%
  let isWin = false;
